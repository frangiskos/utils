/*
 * credits: http://www.fueltravel.com/blog/migrating-from-angular-1-to-2-part-1-pipes/
 *
 * Example use
...
export function orderBy(input: any, [config = '+']): any {
    if (!Array.isArray(input) || input.length === 0) {
        return input;
    }

    // enforce pipe immutability:
    input = [...input];
    ...
    function orderByComparator(a: any, b: any): number {
        // treat null or unefined values as empty strings
        if (a === null) {
            a = '';
        }
        if (b === null) {
